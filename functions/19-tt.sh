...
# the main checkout via cargo. The TS CLI (@towles/tool, bun-global) was
# uninstalled at the ttr->tt cutover (2026-07-13, see towles-tool-rs
# docs/CUTOVER.md); its tmux AgentBoard lives on separately as
# `towles-tool-tmux`/`ttt`.
tt-update() {
  ...
  # `towles-tool` is the Rust CLI's long name — cargo installs only `tt`, and
  # the name used to resolve to the bun-global TS CLI (now towles-tool-tmux).
  ln -sf "$HOME/.cargo/bin/tt" "$HOME/.cargo/bin/towles-tool"
  echo "tt $(tt --version)"
}
